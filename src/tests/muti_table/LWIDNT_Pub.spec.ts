...
import { testData } from "../../scenarios/multi_table/LWIDNT_Pub.scenarios";
...
test.describe(" LWIDNT.W5PA WlookUp Scoring Export Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@LWIDNT", "@MultiTable", "@Regression"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
          await wj5examinerTest_multiTablePage.completeTheTakenTestForWlookUpScoresForLWIDNTTest(
            data.typeOfTest,
            data.BbyC,
          );
        if (
