...
import { testData } from "../../scenarios/multi_table/ORLSMP_Pub.scenarios";
...
test.describe(" STYCMP.W5PA WlookUp Scoring Export Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@STYCMP", "@MultiTable", "@Regression"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
          await wj5examinerTest_multiTablePage.completeTheTakenTestForWlookUpScoresForORLSMPTest(
            data.typeOfTest,
            data.BbyC,
          );
        if (
