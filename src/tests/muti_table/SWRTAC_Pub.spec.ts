...
import { testData } from "../../scenarios/multi_table/SWRTAC_Pub.scenarios";
...
test.describe(" SWRTAC.W5PA WlookUp Scoring Export Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@SWRTAC", "@MultiTable", "@Regression"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
          await wj5examinerTest_multiTablePage.completeTheTakenTestForWlookUpScoresForSWRTACTest(
            data.typeOfTest,
            data.BbyC,
          );
        if (
