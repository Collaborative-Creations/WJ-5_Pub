...
import { testData } from "../../scenarios/multi_table/MATRCZ_Pub.scenarios";
...
test.describe(" MATRCZ.W5PA WlookUp Scoring Export Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@MATRCZ", "@MultiTable", "@Regression"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
          await wj5examinerTest_multiTablePage.completeTheTakenTestForWlookUpScoresForMATRCZTest(
            data.typeOfTest,
            data.BbyC,
          );
        if (
