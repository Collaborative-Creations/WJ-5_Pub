...
import { testData } from "../../scenarios/multi_table/PSGCMP_Pub.scenarios";
...
test.describe(" PSGCMP.W5PA WlookUp Scoring Export Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@PSGCMP", "@MultiTable", "@Regression"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
          await wj5examinerTest_multiTablePage.completeTheTakenTestForWlookUpScoresForPSGCMPTest(
            data.typeOfTest,
            data.BbyC,
          );
        if (
