  ...
  testData.forEach((data) => {
    ...
    test(`@Test_De For ${data.typeOfTest} login as AH Conduct test as Examiner and generate reports`, async ({
      ...
      const { examinee_ID, dateOfBirth } =
        await wj5AhDashPage.addNewExamineeAndUpdateTheTemplate(
          getSiteUrl(),
          data.examineeAge,
          data.location,
          data.testStemForm,
        );
      await wj5AhDashPage.addTestAssignmentBlock(data.blockName, examinee_ID);
      ...
      score =
        await wj5examinerTest_dataExportPage.completeTheTakenTestForTestDataExportforGIWHERPubTest(
          data.typeOfTest,
        );
      if (
