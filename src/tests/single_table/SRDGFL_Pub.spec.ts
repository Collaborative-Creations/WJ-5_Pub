...
import { testData } from "../../scenarios/single_table/SRDGFL_Pub.scenarios";
...
test.describe(" SRDGFL.W5PA WlookUp Scoring Export Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@SRDGFL", "@SingleTable", "@Regression"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
          await wj5examinerTestPage.completeTheTakenTestForWlookUpScoresForSRDGFLTest(
            data.typeOfTest,
          );
        if (
          data.typeOfTest.match(
            /Answer only one sample item|Answer both sample items|Answer only Practice Exercise items/i,
          )
        ) {
