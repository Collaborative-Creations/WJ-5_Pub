...
import { testData } from "../../scenarios/single_table/WRDGFL_Pub.scenarios";
...
test.describe(" WRDGFL.W5PA WlookUp Scoring Export Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@WRDGFL", "@SingleTable", "@Regression"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
          await wj5examinerTestPage.completeTheTakenTestForWlookUpScoresForWRDGFLTest(
            data.typeOfTest,
          );
        if (
          data.typeOfTest.match(
            /Answer only one sample item|Answer both sample items|Answer Sample and Practice Exercise items|Answer only one sample item incorrect|Both Sample Items Incorrect Force Submit Scenario|Practice Exercise Incorrect Force Submit Scenario/i,
          )
        ) {
