...
import { testData } from "../../scenarios/single_table/SWRTFL_Pub.scenarios";
...
test.describe(" SWRTFL.W5PA WlookUp Scoring Export Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      async (
        ...
        score =
          await wj5examinerTestPage.completeTheTakenTestForWlookUpScoresForSWRTFLTest(
            data.typeOfTest,
          );
        if (
          data.typeOfTest.match(
            /Answer only one sample item|Answer two sample items|Answer three sample items/i,
          )
        ) {
