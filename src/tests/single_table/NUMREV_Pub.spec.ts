...
import { testData } from "../../scenarios/single_table/NUMREV_Pub.scenarios";
...
test.describe("NUMREV.W5PA single table lookUp Scoring Export Automation", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      async (
        ...
        score = await wj5examinerTestPage.completeTheTakenTestForWlookUpScoresForNUMREVTest(
          data.typeOfTest,
          data.BbyC,
        );
        if (
          data.typeOfTest.match(
            /Sample Item C Correct Force Submit Scenario/i,
          )
        ) {
