...
import { testData } from "../../scenarios/single_table/SNDBLN_Pub.scenarios";
...
test.describe("SNDBLN.W5PA single table lookUp Scoring Export Automation", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      async (
        ...
        score = await wj5examinerTestPage.completeTheTakenTestForWlookUpScoresForSNDBLNTest(
          data.typeOfTest,
          data.BbyC,
        );
        if (data.typeOfTest.match(/Answer only one sample item/i)) {
