...
import { testData } from "../../scenarios/single_table/MTHFLU_Pub.scenarios";
...
test.describe(" MTHFLU.W5PA WlookUp Scoring Export Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@MTHFLU", "@SingleTable", "@Regression"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
          await wj5examinerTestPage.completeTheTakenTestForWlookUpScoresForMTHFLUTest(
            data.typeOfTest,
          );
        if (data.typeOfTest.match(/Answer only Practice Exercise/i)) {
