...
import { testData } from "../../scenarios/derived_scores(compounds & clusters)/GL_Pub_cluster.scenarios";
...
test.describe(" GL cluster Derived Export Automation ", () => {
  testData.forEach((data) => {
    test(
      `For ${data.typeOfTest} Complete The GL cluster & generate report`,
      { tag: ["@GL", "@Regression", "@DerivedScores"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        for (let i = 0; i < data.testName.length; i++) {
          ...
          score =
            await wj5examnrTest_derivedScoresPage.completeTheTakenTestForDerivedScoresForGLCluster(
              data.typeOfTest[i],
              data.testStemForm[i],
              data.BbyC![i],
              data.scoreFlag,
