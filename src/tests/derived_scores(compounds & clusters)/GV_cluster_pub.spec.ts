...
import { testData } from "../../scenarios/derived_scores(compounds & clusters)/GV_Pub_cluster.scenarios";
...
test.describe(" GV cluster Derived Export Automation ", () => {
    ...
    test(
      ...
      async (
        ...
        for (let i = 0; i < data.testName.length; i++) {
          ...
          score =
            await wj5examnrTest_derivedScoresPage.completeTheTakenTestForDerivedScoresForGVCluster(
              data.typeOfTest[i],
              data.testStemForm[i],
              data.BbyC![i],
              data.scoreFlag,
