...
import { testData } from "../../scenarios/derived_scores(compounds & clusters)/PHNMAN_Pub_cluster.scenarios";
...
test.describe(" PHNMAN cluster Derived Export Automation ", () => {
    ...
    test(
      ...
      async (
        ...
        for (let i = 0; i < data.testName.length; i++) {
          ...
          score =
            await wj5examnrTest_derivedScoresPage.completeTheTakenTestForDerivedScoresForPHNMANCluster(
              data.typeOfTest[i],
              data.testStemForm[i],
              data.BbyC![i],
              data.scoreFlag,
