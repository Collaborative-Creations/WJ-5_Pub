...
import { testData } from "../../scenarios/derived_scores(compounds & clusters)/ORLEXP_Pub_cluster.scenarios";
...
test.describe(" ORLEXP cluster Derived Export Automation ", () => {
    ...
    test(
      ...
      async (
        ...
        for (let i = 0; i < data.testName.length; i++) {
          ...
          score =
            await wj5examnrTest_derivedScoresPage.completeTheTakenTestForDerivedScoresForORLEXPCluster(
              data.typeOfTest[i],
              data.testStemForm[i],
              data.BbyC![i],
              data.scoreFlag,
