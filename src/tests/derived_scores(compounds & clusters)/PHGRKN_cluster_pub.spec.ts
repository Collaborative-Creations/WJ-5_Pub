...
import { testData } from "../../scenarios/derived_scores(compounds & clusters)/PHGRKN_Pub_cluster.scenarios";
...
test.describe(" PHGRKN cluster Derived Export Automation ", () => {
    ...
    test(
      ...
      async (
        ...
        for (let i = 0; i < data.testName.length; i++) {
          ...
          score =
            await wj5examnrTest_derivedScoresPage.completeTheTakenTestForDerivedScoresForPHGRKNCluster(
              data.typeOfTest[i],
              data.testStemForm[i],
              data.BbyC![i],
              data.scoreFlag,
