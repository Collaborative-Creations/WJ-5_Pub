...
import { testData } from "../../scenarios/derived_scores(compounds & clusters)/SPLSKL_Pub_cluster.scenarios";
...
test.describe(" SPLSKL cluster Derived Export Automation ", () => {
    ...
    test(
      ...
      async (
        ...
        for (let i = 0; i < data.testName.length; i++) {
          ...
          score =
            await wj5examnrTest_derivedScoresPage.completeTheTakenTestForDerivedScoresForSPLSKLCluster(
              data.typeOfTest[i],
              data.testStemForm[i],
              data.BbyC![i],
              data.scoreFlag,
