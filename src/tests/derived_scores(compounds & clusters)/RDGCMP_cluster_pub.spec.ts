...
import { testData } from "../../scenarios/derived_scores(compounds & clusters)/RDGCMP_Pub_cluster.scenarios";
import { getExamineeURL, getSiteUrl } from "../../utils/testData";
import { setFilePathes } from "../../utils/global";
import { ScoreObject, ExamineeData } from "../../utils/types";
...
test.describe(" RDGCMP cluster Derived Export Automation ", () => {
  testData.forEach((data) => {
    test(
      `For ${data.typeOfTest} Complete The RDGCMP cluster & generate report`,
      { tag: ["@RDGCMP", "@Regression", "@DerivedScores"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        for (let i = 0; i < data.testName.length; i++) {
          ...
          score =
            await wj5examnrTest_derivedScoresPage.completeTheTakenTestForDerivedScoresForRDGCMPCluster(
              data.typeOfTest[i],
              data.testStemForm[i],
              data.BbyC![i],
              data.scoreFlag,
