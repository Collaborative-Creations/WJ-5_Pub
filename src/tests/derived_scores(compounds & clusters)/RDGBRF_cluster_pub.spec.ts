...
import { testData } from "../../scenarios/derived_scores(compounds & clusters)/RDGBRF_Pub_cluster.scenarios";
...
test.describe(" RDGBRF cluster Derived Export Automation ", () => {
  testData.forEach((data) => {
    test(
      `For ${data.typeOfTest} Complete The RDGBRF cluster & generate report`,
      { tag: ["@RDGBRF", "@Regression", "@DerivedScores"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        for (let i = 0; i < data.testName.length; i++) {
          ...
          score =
            await wj5examnrTest_derivedScoresPage.completeTheTakenTestForDerivedScoresForRDGBRFCluster(
              data.typeOfTest[i],
              data.testStemForm[i],
              data.BbyC![i],
              data.scoreFlag,
