...
import { testData } from "../../scenarios/derived_scores(compounds & clusters)/RDGFLU_Pub_cluster.scenarios";
...
test.describe(" RDGFLU cluster Derived Export Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      async (
        ...
        /**
         * The retry has been added to adreess the issue where sometimes even after adding the examinee the test assignment is not created
         */
        ...
        for (let i = 0; i < data.testName.length; i++) {
          ...
          score =
            await wj5examnrTest_derivedScoresPage.completeTheTakenTestForDerivedScoresForRDGFLUCluster(
              data.typeOfTest[i],
              data.testStemForm[i],
              data.BbyC![i],
              data.scoreFlag,
