...
import { testData } from "../../scenarios/Left_Nav/SEGMNT_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@SEGMNTLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
          await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForSEGMNTTest(
            data.typeOfTest,
          );
        if (data.typeOfTest.match(/SSP Item check For SSP2|left Nav When Items are Flagged,Expanded and collapsed For SSP3/i)) {
