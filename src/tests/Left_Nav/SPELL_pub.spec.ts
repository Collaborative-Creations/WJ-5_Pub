...
import { testData } from "../../scenarios/Left_Nav/SPELL_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@SPELLLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
        await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForSPELLTest(
            data.typeOfTest,
          );
          if (
            data.typeOfTest.match(
              /SSP Item check For SSP3|SSP Item check For SSP4|SSP Item check For SSP5|SSP Item check For SSP6|SSP Item check For SSP7|left Nav When Items are Flagged,Expanded and collapsed For SSP8/i,
            )
          ) {
