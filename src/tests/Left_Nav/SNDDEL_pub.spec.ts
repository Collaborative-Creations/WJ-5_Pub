...
import { testData } from "../../scenarios/Left_Nav/SNDDEL_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@SNDDELLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
        await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForSNDDELTest(
            data.typeOfTest,
          );
          if (
            data.typeOfTest.match(
                /SSP item check for SNDDEL SSP2|Flagged,expanded and collapsed for SNDDEL SSP3/i,                             
            )
          ) {
