...
import { testData } from "../../scenarios/Left_Nav/VRBATN_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@VRBATNLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
        await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForVRBATNTest(
            data.typeOfTest,
          );
          if (
            data.typeOfTest.match(
              /SSP item check for MPRBID SSP3|SSP item check for MPRBID SSP4|SSP item check for MPRBID SSP5|Flagged,expanded and collapsed for MPRBID SSP6/i
