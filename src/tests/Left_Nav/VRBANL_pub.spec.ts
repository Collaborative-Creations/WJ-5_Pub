...
import { testData } from "../../scenarios/Left_Nav/VRBANL_pub_scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@VRBANLLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
        await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForVRBANLTest(
            data.typeOfTest,
          );
          if (
            data.typeOfTest.match(
              /SSP item check for VRBANL SSP3|SSP item check for VRBANL SSP4|Flagged,expanded and collapsed for VRBANL SSP5/i
