...
import { testData } from "../../scenarios/Left_Nav/ORLCMP_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@ORLCMPLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
        await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForORLCMPTest(
            data.typeOfTest,
          );
          if (
            data.typeOfTest.match(
              /SSP Item check For ORLCMP SSP3|SSP Item check For ORLCMP SSP4|SSP Item check For ORLCMP SSP5|left Nav When Items are Flagged,Expanded and collapsed For ORLCMP SSP6/i,
            )
          ) {
