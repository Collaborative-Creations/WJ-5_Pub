...
import { testData } from "../../scenarios/Left_Nav/RPDPIC_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@RPDPICLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
          await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForRPDPICTest(
            data.typeOfTest,
          );
          if (data.typeOfTest.match(/All correct scenario for RPDPIC|All Incorrect scenario for RPDPIC/i)) {
