...
import { testData } from "../../scenarios/Left_Nav/LWIDNT_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@LWIDNTLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
        await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForLWIDNTTest(
            data.typeOfTest,
          );
          if (
            data.typeOfTest.match(
              /SSP item check for LWIDNT SSP3|SSP item check for LWIDNT SSP4|SSP item check for LWIDNT SSP5|Flagged,expanded and collapsed for LWIDNT SSP6/i
