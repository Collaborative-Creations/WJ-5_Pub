...
import { testData } from "../../scenarios/Left_Nav/APPROB_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@APPROBLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
        await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForAPPROBTest(
            data.typeOfTest,
          );
          if (
            data.typeOfTest.match(
              /SSP item check for APPROB SSP3|SSP item check for APPROB SSP4|SSP item check for APPROB SSP5|SSP item check for APPROB SSP6|SSP item check for APPROB SSP7|Flagged,expanded and collapsed for APPROB SSP8/i
