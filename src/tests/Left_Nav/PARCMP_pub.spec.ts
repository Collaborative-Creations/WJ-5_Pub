...
import { testData } from "../../scenarios/Left_Nav/PARCMP_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@PARCMPLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
        await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForPARCMPTest(
            data.typeOfTest,
          );
          if (
            data.typeOfTest.match(
              /Flagged,expanded and collapsed for PARCMP SSP4/i                 
            )
          ) {
