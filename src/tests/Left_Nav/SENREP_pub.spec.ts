...
import { testData } from "../../scenarios/Left_Nav/SENREP_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@SENREPLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
          await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForSENREPTest(
            data.typeOfTest,
          );
        if (data.typeOfTest.match(/SSP item check for SSP3|SSP item check for SSP4|SSP item check for for SSP5|left Nav When Items are Flagged,Expanded and collapsed for SSP6/i)) {
