...
import { testData } from "../../scenarios/Left_Nav/SPAREL_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@SPARELLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
        await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForSPARELTest(
            data.typeOfTest,
          );
          if (
            data.typeOfTest.match(
              /Flagged,expanded and collapsed for SPAREL SSP2/i,
            )
          ) {
