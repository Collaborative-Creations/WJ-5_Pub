...
import { testData } from "../../scenarios/Left_Nav/GIWHAT&GIWHER_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@GIWHATandGIWHER", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
          await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForGIWHATandGIWHERTests(
            data.typeOfTest,
          );
        if (
          data.typeOfTest.match(
            /left Nav when items are flagged for SSP2|left Nav when expanded and collapsed for SSP3|left Nav when items are flagged,expanded and collapsed for SSP5|Flagged Items for GIWHER SSP2|Flagged,expanded and collapsed for GIWHER SSP3/i,
          )
        ) {
