...
import { testData } from "../../scenarios/Left_Nav/WRTSMP_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@WRTSMPLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
          await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForWRTSMPTest(
            data.typeOfTest,
          );
        if (
          data.typeOfTest.match(
            /SSP Item check for SSP 2|left Nav When Items are Flagged,Expanded and collapsed For SSP3/i,
          )
        ) {
