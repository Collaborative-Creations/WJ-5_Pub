...
import { testData } from "../../scenarios/Left_Nav/NUMREV_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@NUMREVLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
        await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForNUMREVTest(
            data.typeOfTest,
          );
          if (
            data.typeOfTest.match(
              /Flagged,expanded and collapsed for NUMREV SSP3/i,
            )
          ) {
