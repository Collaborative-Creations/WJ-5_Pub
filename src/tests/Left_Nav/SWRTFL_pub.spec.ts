...
import { testData } from "../../scenarios/Left_Nav/SWRTFL_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@SWRTFLLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
        await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForSWRTFLTest(
            data.typeOfTest,
          );
          if (
            data.typeOfTest.match(
              /Flagged,expanded and collapsed for SNDSUB SSP2/i,
            )
          ) {
