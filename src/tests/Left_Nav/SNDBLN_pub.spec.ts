...
import { testData } from "../../scenarios/Left_Nav/SNDBLN_pub.scenarios";
  ...
  testData.forEach((data) => {
    ...
    test(
      ...
      async (
        ...
        score =
        await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForSNDBLNTest(
            data.typeOfTest,
          );
          if (
            data.typeOfTest.match(
              /left Nav when items are flagged for SSP2|left Nav When Items are Flagged,Expanded and collapsed For SSP3/i,
            )
          ) {
