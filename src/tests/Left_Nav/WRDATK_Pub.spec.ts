...
import { testData } from "../../scenarios/Left_Nav/WRDATK_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@WRDATKLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
        await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForWRDATKTest(
            data.typeOfTest,
          );
          if (
            data.typeOfTest.match(
              /SSP item check for WRDATK SSP3|SSP item check for WRDATK SSP4|SSP item check for WRDATK SSP5|SSP item check for WRDATK SSP6|Flagged,expanded and collapsed for WRDATK SSP7/i
