...
import { testData } from "../../scenarios/Left_Nav/STYREC_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@STYRECLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
        await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForSTYRECTest(
            data.typeOfTest,
          );
          if (
            data.typeOfTest.match(
              /SSP item check for STYREC SSP2|Flagged,expanded and collapsed for STYREC SSP3/i           
            )
          ) {
