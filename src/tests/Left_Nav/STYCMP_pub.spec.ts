...
import { testData } from "../../scenarios/Left_Nav/STYCMP_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@STYCMPLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
        await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForSTYCMPTest(
            data.typeOfTest,
          );
          if (
            data.typeOfTest.match(
              /Flagged,expanded and collapsed for STYCMP SSP4/i                 
            )
          ) {
