...
import { testData } from "../../scenarios/Left_Nav/SWRTAC_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@SWRTACLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
        await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForSWRTACTest(
            data.typeOfTest,
          );
          if (
            data.typeOfTest.match(
              /Left Nav for Block A|Left Nav for Block B|Left Nav for Block C/i,
            )
          ) {
