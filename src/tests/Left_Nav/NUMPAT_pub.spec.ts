...
import { testData } from "../../scenarios/Left_Nav/NUMPAT_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@NUMPATLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
        await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForNUMPATTest(
            data.typeOfTest,
          );
          if (
            data.typeOfTest.match(
              /SSP Item check For SSP3|SSP Item check For SSP4|left Nav When Items are Flagged,Expanded and collapsed For SSP5/i,
            )
          ) {
