...
import { testData } from "../../scenarios/Left_Nav/CALC_pub.scenarios";
  ...
  testData.forEach((data) => {
    ...
    test(
      ...
      async (
        ...
        score =
        await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForCALCTest(
            data.typeOfTest,
          );
          if (
            data.typeOfTest.match(
              /SSP Item check For SSP3|SSP Item check For SSP4|SSP Item check For SSP5|SSP Item check For SSP6|left Nav When Items are Flagged,Expanded and collapsed For SSP7/i,
            )
          ) {
