...
import { testData } from "../../scenarios/Left_Nav/ORLSMP_pub.scenarios";
...
test.describe(" LeftNav Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      { tag: ["@Regression", "@ORLSMPLN", "@leftnav"] },
      async (
        {
          wj5ah,
          wj5AhDashPage,
        ...
        score =
          await wj5examinerTest_leftnavPage.completeTheLeftNavValidationForORLSMPTest(
            data.typeOfTest,
          );
        if (
          data.typeOfTest.match(
            /SSP Item Check ORLSMP for SSP2|left Nav When Items are Flagged,Expanded and collapsed For SSP3/i,
          )
        ) {
