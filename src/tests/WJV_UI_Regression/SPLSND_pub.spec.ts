...
import { testData } from "../../scenarios/WJV_UI_Regression/SPLSND_pub.scenarios";
...
test.describe(" SPLSND UI Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      async (
        ...
        score =
          await wj5examinerTest_SPLSNDPage.completeTheTakenTestForSPLSNDTest(
            data.typeOfTest,
          );
        if (
          data.typeOfTest.match(
            /Block A Review Flow Logic|Block B Review Flow Logic/i,
          )
        ) {
