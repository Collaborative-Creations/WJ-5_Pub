...
import { testData } from "../../scenarios/WJV_UI_Regression/SRDGFL_Pub.scenarios";
...
test.describe(" SRDGFL UI Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      async (
        ...
        score =
          await wj5examinerTest_SRDGFLPage.completeTheTakenTestForSRDGFLTest(
            data.typeOfTest,
          );
        if (
          data.typeOfTest.match(
            /Practice Exercise All correct scenario|Practice Exercise 3 Correct 1 Incorrect scenario/i,
          )
        ) {
