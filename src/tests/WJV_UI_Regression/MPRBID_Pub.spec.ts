...
import { testData } from "../../scenarios/WJV_UI_Regression/MPRBID_Pub.scenarios";
...
test.describe(" MPRBID UI Automation ", () => {
      ...
      async (
        {
          ...
          wj5ExaminerDashPage,
          wj5examinerTestPage,
          wj5examinerTest_mprbidPage,
          browser,
        },
        testInfo
      ) => {
        test.setTimeout(36 * 60 * 1000);
        ...
        score = await wj5examinerTest_mprbidPage.completeTheTakenTestForMPRBID(
          data.typeOfTest
        );
        if (data.typeOfTest.match(/Block A Forward Flow when all correct|Block B Forward Flow when RS is 4|Block B Forward Flow when RS is 4(ssp2)/i)) {
