...
import { testData } from "../../scenarios/WJV_UI_Regression/WRTSMP_Pub.scenarios";
...
test.describe(" WRTSMP UI Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      async (
        ...
        score = await wj5examinerTest_wrtsmpPage.completeTheTakenTestForWRTSMP(
          data.typeOfTest,
        );
        if (
          data.typeOfTest.match(
            /Block A Test Forward Scenario|Block A&B End Test Scenario With RS:14|Review Flow Check from End Test to Intro 3 for SSP1/i,
          )
        ) {
