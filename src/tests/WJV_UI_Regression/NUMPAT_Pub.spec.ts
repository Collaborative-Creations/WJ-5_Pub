...
import { testData } from "../../scenarios/WJV_UI_Regression/NUMPAT_Pub.scenarios";
...
test.describe(" NUMPAT UI Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      async (
        ...
        score =
          await wj5examinerTest_NUMPATPage.completeTheTakenTestForNUMPATTest(
            data.typeOfTest,
          );
        if (
          data.typeOfTest.match(
            /Practice Exercise All correct scenario|Practice Exercise 5 Correct 1 Incorrect scenario/i,
          )
        ) {
