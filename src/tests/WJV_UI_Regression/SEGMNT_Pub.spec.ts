...
import { testData } from "../../scenarios/WJV_UI_Regression/SEGMNT_Pub.scenarios";
...
test.describe(" SEGMNT UI Regression ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      async (
        ...
        score = await wj5examinerTest_segmntPage.completeTheTakenTestForSEGMNT(
          data.typeOfTest,
        );
        if (
          data.typeOfTest.match(
            /Review Flow Check from End Test for SSP1/i,
          )
        ) {
