...
import { testData } from "../../scenarios/Rerouting/rerouting_pub.scenarios";
...
test.describe(" Rerouting Automation ", () => {
  testData.forEach((data) => {
    ...
    test(
      ...
      async (
        ...
        score =
          await wj5examinerTest_reroutingPage.completeTheReroutingTestingforAllPossibleTests(
            data.typeOfTest,
          );
        if (
          data.typeOfTest.match(
            /NUMPAT Sample Items End Test Scenario|OVSYN next item check Scenario|OVANT next item check when response changed Scenario|PICVOC next item check when response changed Scenario|APPROB lowest numbered unadministered item rerouting Scenario|LETPAT Sample Items End Test Scenario|SNDDEL Block A End Test Scenario|SNDDEL Block B End Test Scenario|SNDDEL Block C End Test Scenario/i,
          )
        ) {
