...
export default class Wj5DashboardPage {
  ...
  examiner = this.userData.examiner[0];
  accountHolder = this.userData.accountHolder[0];
  ...
  async addNewExamineeAndUpdateTheTemplate(
    ...
    await relogging.reloginIfneeded(this.examiner.userName, this.examiner.passWord);
  ...
  async addTestAssignmentBlock(blockName: string, studentID: string) {
    ...
    await this.selectExaminer(examiner.examinerID);
  ...
  async welcomeTextToBeVisable() {
    await this.page.bringToFront();
    const relogging = new Wj5LoginPage(this.page);
    await relogging.reloginIfneeded(this.accountHolder.userName, this.accountHolder.passWord);
  ...
  async clickOnTheRecentExamineeOnceFullyLoaded(studentID: string) {
    ...
    await relogging.reloginIfneeded(
      this.examiner.userName,
      this.examiner.passWord,
    );
  ...
  async examinerRelogin(studentID: string) {
    ...
    await this.page.getByPlaceholder("Username").fill(this.examiner.userName);
    await this.page.getByPlaceholder("Username").fill(this.examiner.userName);
    await this.page.getByPlaceholder("Password").click();
    await this.page.getByPlaceholder("Password").fill(this.examiner.passWord);
    await this.page.getByPlaceholder("Password").fill(this.examiner.passWord);
