...
export default class wj5SentenceWritingFluencyPage {
  ...
  private readonly stopTimerIntroText: Locator;
  private readonly stopTimerAlertMessage: Locator;
  private readonly stopTimerButton: Locator;
  private readonly stopTimerWarningPopUP: Locator;
  private readonly stopTimerWarningPopUPMessage: Locator;
  private readonly cancelButtonInstopTimerPopup: Locator;
  private readonly confirmStopTimerButton: Locator;
  ...
  constructor(page: Page) {
    ...
    this.stopTimerIntroText = this.page.locator("//div[@class = 'intro-text']//p");
    this.stopTimerAlertMessage = this.page.locator("(//div[@class = 'intro-text']//p)[3]");
    this.stopTimerButton = this.page.locator("//div[@class = 'intro-text']//button[text()='Stop Timer']");
    this.stopTimerWarningPopUP = this.page.locator("//div[@class = 'intro-text']//h3[text()='Stop Timer Warning']");
    this.stopTimerWarningPopUPMessage = this.page.locator("//div[@class = 'intro-text']//h4");
    this.cancelButtonInstopTimerPopup = this.page.locator("//button[text()='Cancel']");
    this.confirmStopTimerButton = this.page.locator("//button[text()='Confirm Stop Timer']");
  ...
  async completeTheTakenTestForSWRTFLTest(
    typeOfTest: string,
    ...
    while (await this.plainNextButtonOrEndButton.isVisible()) {
      ...
      if (typeOfTest.match(/Sample Items End Test Flow scenario/i)) {
        ...
        if (itemDetails.startsWith("Sample")) {
          ...
          expect(await this.listOfPagesAndTotalText.nth(0).textContent()).toEqual("Page 2:");
          expect(await this.listOfPagesAndTotalText.nth(1).textContent()).toEqual("Page 3:");
          expect(await this.listOfPagesAndTotalText.nth(2).textContent()).toEqual("Page 4:");
          expect(await this.listOfPagesAndTotalText.nth(3).textContent()).toEqual("Page 5:");
          expect(await this.listOfPagesAndTotalText.nth(4).textContent()).toEqual("Page 6:");
          expect(await this.listOfPagesAndTotalText.nth(5).textContent()).toEqual("Total:");
          for (let i = 0; i <= 4; i++) {
          ...
          expect(await this.totalScore.nth(5).textContent()).toContain("0");
          await this.scoreLaterButton.isEnabled();
          await this.submitButton.click();
        }
      } else if (typeOfTest.match(/Page Score Entry With All Eight/i)) {
        if (itemDetails.startsWith("Sample")) {
          await correctlocator.click();
        } else if (itemDetails.startsWith("Test Introduction")) {
          await this.startPracticeAndStartTimerButton.click();
          ...
          for (let i = 0; i <= 4; i++) {
            await this.listOfScores.nth(i).click();
            await this.listOfScores.nth(i).fill("8");
          }
          await this.submitButton.click();
        }
        ...
        if (itemDetails.startsWith("Sample")) {
          ...
          await this.listOfScores.nth(5).click();
          await this.listOfScores.nth(5).fill("25");
          for (let i = 0; i <= 4; i++) {
            await expect(this.listOfScores.nth(i)).toHaveAttribute("disabled", "");
          }
          await this.closeScoreFields.nth(1).click();
          for (let j = 0; j <= 4; j++) {
            await this.listOfScores.nth(j).isEnabled();
          }
          await this.pencilIcon.nth(0).click();
          await this.listOfScores.nth(5).click();
          await this.listOfScores.nth(5).fill("20");
          await this.submitButton.isEnabled();
          await this.scoreLaterButton.isEnabled();
          await this.submitButton.click();
        }
        ...
        if (itemDetails.startsWith("Sample")) {
          ...
          await this.pencilIcon.nth(0).click();
          await this.listOfScores.nth(5).click();
          await this.listOfScores.nth(5).fill("30");
          await this.submitButton.click();
          await this.editScoresLink.click();
          await this.closeScoreFields.nth(1).click();
          for (let i = 0; i <= 4; i++) {
            await this.listOfScores.nth(i).click();
            await this.listOfScores.nth(i).isEnabled();
            await this.listOfScores.nth(i).fill("2");
            if (i == 4) {
              await this.submitButton.isEnabled();
            } else {
              await expect(this.submitButton).toHaveAttribute("disabled", "");
      ...
      } else if (typeOfTest.match(/Page Score Entry After Clicking On Stop Timer Button/i)) {
        if (itemDetails.startsWith("Sample")) {
          await correctlocator.click();
        } else if (itemDetails.startsWith("Test Introduction")) {
          await this.startPracticeAndStartTimerButton.click();
          ...
          await this.stopTimerButton.isEnabled();
          await this.stopTimerButton.click();
          await expect(this.stopTimerWarningPopUP).toBeVisible();
          expect(await this.stopTimerWarningPopUP.textContent()).toEqual("Stop Timer Warning");
          expect(await this.stopTimerWarningPopUPMessage.nth(0).textContent()).toContain("If you stop the timer, this test will end and you will not be able to resume.");
          await expect(this.confirmStopTimerButton).toBeVisible();
          await expect(this.cancelButtonInstopTimerPopup).toBeVisible();
          await this.confirmStopTimerButton.isEnabled();
          await this.cancelButtonInstopTimerPopup.isEnabled();
          await this.confirmStopTimerButton.click();
          for (let i = 0; i <= 4; i++) {
            await expect(this.listOfScores.nth(i)).toHaveAttribute("value", "0");
          }
          await this.submitButton.isEnabled();
          expect(await this.timeUpView.textContent()).toEqual("Time is up.");
          for (let j = 0; j <= 4; j++) {
            await this.listOfScores.nth(j).click();
            await this.listOfScores.nth(j).fill("5");
          }
          await this.submitButton.click();
        }
      } else if (typeOfTest.match(/Page Score Entry After Clicking On Cancel Button/i)) {
        ...
        if (itemDetails.startsWith("Sample")) {
          ...
          await this.stopTimerAlertMessage.waitFor({ state: "visible" });
          await this.stopTimerButton.click();
          await this.cancelButtonInstopTimerPopup.click();
          await this.timeUpView.waitFor({ state: "visible" });
          await this.pencilIcon.nth(0).click();
          await this.listOfScores.nth(5).click();
          await this.listOfScores.nth(5).fill("20");
        ...
        if (itemDetails.startsWith("Sample")) {
          ...
          expect(await this.totalScore.nth(5).textContent()).toContain("10");
          ...
          for (let j = 0; j <= 4; j++) {
            await this.listOfScores.nth(j).click();
            await this.listOfScores.nth(j).isEnabled();
            await this.listOfScores.nth(j).fill("3");
            if (j == 4) {
              await this.submitButton.isEnabled();
            } else {
              await expect(this.submitButton).toHaveAttribute("disabled", "");
          ...
          await this.pencilIcon.nth(0).click();
          await this.listOfScores.nth(5).click();
          await this.listOfScores.nth(5).clear();
          await this.listOfScores.nth(5).fill("30");
          await this.submitButton.click();
        }
        ...
        if (itemDetails.startsWith("Sample")) {
          ...
          for (let i = 0; i <= 4; i++) {
            await this.listOfScores.nth(i).click();
            await this.listOfScores.nth(i).fill("5");
          }
          await this.submitButton.click();
