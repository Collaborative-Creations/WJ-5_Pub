...
export default class wj5GeneralInfoPage {
  ...
  async completeTheTakenTestForGIWHATTest(
    typeOfTest: string,
    ...
    while (await this.plainNextButtonOrEndButton.isVisible()) {
    ...
    } else if (typeOfTest.match(/Reverse logic with 4 incorrect scenario SSP4/i)) {
        if (["Introduction 1"].includes(itemDetails)) {
        } else if (itemDetails.includes("Item 8")) {
          await incorrectlocator.click();
      ...
      } else if (typeOfTest.match(/Reverse logic with 4 incorrect scenario SSP5/i)) {
        if (["Introduction 1"].includes(itemDetails)) {
        } else if (itemDetails.includes("Item 11")) {
          await incorrectlocator.click();
          await expect.soft(this.nextText).toHaveText("Item 7");
          await this.plainNextButtonOrEndButton.click();
          await incorrectlocator.click();
          await expect.soft(this.nextText).toHaveText("Item 3");
          await this.plainNextButtonOrEndButton.click();
          await incorrectlocator.click();
          await expect.soft(this.nextText).toHaveText("Item 1");
        } else if (itemDetails.startsWith("Item")) {
