...
export default class wj5SpellPage {
  ...
  async completeTheTakenTestForSPELL(
    typeOfTest: string
    ...
    while (await this.plainNextButtonOrEndButton.isVisible()) {
      ...
      }else if (typeOfTest.match(/All Correct SSP (1|2|3|4|5|6|7|8)/i)) { 
          await correctlocator.click();
      }else if (/Reverse Logic for (SSP2|SSP3|SSP4|SSP5|SSP6|SSP7|SSP8)/i.test(typeOfTest)) {
        const testType = typeOfTest.match(/SSP2|SSP3|SSP4|SSP5|SSP6|SSP7|SSP8/i)?.[0];
        const itemNumber = parseInt(itemDetails.match(/^Item (\d+)\b/)?.[1], 10);
      
        if (testType && itemNumber) {
          const conditions = {
            SSP2: { dontKnow: [7, 2, 1, ...Array.from({ length: 3 }, (_, i) => 3 + i)] },
            SSP3: { dontKnow: [15, 10, 5, 1, ...Array.from({ length: 3 }, (_, i) => 2 + i)] },
            SSP4: { dontKnow: [20, 15, 10, 5, 1, ...Array.from({ length: 3 }, (_, i) => 2 + i)] },
            SSP5: { dontKnow: [24, 19, 14, 9, 4, 1, ...Array.from({ length: 3 }, (_, i) => (i === 2 ? 5 : 2 + i))] },
            SSP6: { dontKnow: [30, 25, 20, 15, 10, 5, 1, ...Array.from({ length: 3 }, (_, i) => 2 + i)] },
            SSP7: { dontKnow: [33, 28, 23, 18, 13, 8, 3, 1, ...Array.from({ length: 3 }, (_, i) => (i === 1 ? 4 : i === 2 ? 5 : 2 + i))] },
            SSP8: { dontKnow: [43, 38, 33, 28, 23, 18, 13, 8, 3, 1, ...Array.from({ length: 3 }, (_, i) => (i === 1 ? 4 : i === 2 ? 5 : 2 + i))] },
      ...
      }else if (/B&C (SSP1|SSP2|SSP3|SSP4|SSP5|SSP6|SSP7|SSP8)/i.test(typeOfTest)) {
        const testType = typeOfTest.match(/SSP1|SSP2|SSP3|SSP4|SSP5|SSP6|SSP7|SSP8/i)?.[0];
        const itemNumber = parseInt(itemDetails.match(/^Item (\d+)\b/)?.[1], 10);
        
        if (testType && itemNumber) {
            const ranges = {
              SSP1: { correctRange: [1, 2, 3, 4, 5], dontKnowRange: [6, 7, 8, 9, 10] },
              SSP2: { correctRange: [7, 8, 9, 10, 11], dontKnowRange: [12, 13, 14, 15, 16] },
              SSP3: { correctRange: [15, 16, 17, 18, 19], dontKnowRange: [20, 21, 22, 23, 24] },
              SSP4: { correctRange: [20, 21, 22, 23, 24], dontKnowRange: [25, 26, 27, 28, 29] },
              SSP5: { correctRange: [24, 25, 26, 27, 28], dontKnowRange: [29, 30, 31, 32, 33] },
              SSP6: { correctRange: [30, 31, 32, 33, 34], dontKnowRange: [35, 36, 37, 38, 39] },
              SSP7: { correctRange: [33, 34, 35, 36, 37], dontKnowRange: [38, 39, 40, 41, 42] },
              SSP8: { correctRange: [43, 44, 45, 46, 47], dontKnowRange: [48, 49, 50, 51, 52] },
            };
