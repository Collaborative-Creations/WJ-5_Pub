...
export default class wj5MemoryForWordsPage {
  ...
  async completeTheTakenTestForMEMWRDTest(
    typeOfTest: string,
    ...
    while (await this.plainNextButtonOrEndButton.isVisible()) {
      ...
      if (typeOfTest.match(/Basal and ceiling with 4 incorrect scenario/i)) {
      ...
      } else if (typeOfTest.match(/Reverse logic with 4 correct and 4 incorrect scenario/i)) {
        if (itemDetails.startsWith("Sample")) {
          await correctlocator.click();
        } else if (itemDetails.startsWith("Item")) {
          for (let i = 1; i <= 2; i++) {
        ...
        if (itemDetails.startsWith("Sample")) {
          await correctlocator.click();
        } else if (/^Item (1|2|6)\b/.test(itemDetails)) {
          await incorrectlocator.click();
        } else if (itemDetails === 'Item 3') {
          await incorrectlocator.click();
          await this.FlagIcon.click();
