...
export default class wj5RapidPhonemeNamingPage {
  ...
  constructor(page: Page) {
    ...
    this.itemScoreValue = this.page.locator(
      "//div[@class='select-group item-score-group']//child::div[@class='item-score']"
    );
    this.rawScoreText = this.page.locator("//div[@class='raw-score']");
    this.endTest = this.page.locator("//button[text()='End Test']");
    this.listOfPracticeExerciseItems = this.page.locator(
      "//div[@class='examinee-view']//child::div[@class='fluency-mc']"
    );
    this.introductionError = this.page.locator(
      "//div[@class='introduction error']//child::*[@class='error-icon']"
    );
    this.inCorrectAnswerErrorMessage = this.page.locator(
      "(//div[@class='intro-text']//child::span)"
    );
    ...
    this.closeBtnTimer = this.page.locator(
      "//div[@class='ReactModal__Content ReactModal__Content--after-open popup-infopage-modal']//button[text()='Close']"
    );
  ...
  async completeTheTakenTestForRPDPHOTest(
    typeOfTest: string
    ...
    while (await this.plainNextButtonOrEndButton.isVisible()) {
     ...
     if (
        ...
        typeOfTest.match(/Test Items 5 correct and 5 incorrect scenario/i)
      ) {
        if (itemDetails.startsWith("Sample")) {
          await this.doneButton.click();
        } else if (itemDetails.startsWith("Item Set")) {
          await this.listOfInCorrectAnswers.nth(0).click();
        ...
        typeOfTest.match(/When Timer Ran Out for First Item set/i)
      ) {
        if (itemDetails.startsWith("Sample")) {
          await this.doneButton.click();
        ...
        typeOfTest.match(/When Timer Ran Out for Last Item set/i)
      ) {
        if (itemDetails.startsWith("Sample")) {
          await this.doneButton.click();
          ...
          await this.listOfInCorrectAnswers.nth(11).click();
          await this.continueButton.click();
          break;
        }
      }  else {
