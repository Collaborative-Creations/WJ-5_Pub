...
export default class wj5CalculationPage {
  ...
  async completeTheTakenTestForCALCTest(
    typeOfTest: string,
    ...
    while (await this.plainNextButtonOrEndButton.isVisible()) {
      ...
      } else if (typeOfTest.match(/Basal and ceiling with 5 incorrect scenario/i)) {
        if (itemDetails.startsWith("Sample")) {
      ...
      } else if (typeOfTest.match(/Review flow Scenario for SSP2|Review flow Scenario for SSP3|Review flow Scenario for SSP5|Review flow Scenario for SSP6|Review flow Scenario for SSP7/i)) {
        if (itemDetails.startsWith("Sample")) {
          await correctlocator.click();
        } else if (/^Item ([1-3]|[6-9]|1[1-4]|1[7-9]|22|23|27)\b/.test(itemDetails)) {
          await incorrectlocator.click();
        } else if (itemDetails === 'Item 4') {
          await incorrectlocator.click();
          await this.FlagIcon.click();
      ...
      } else if (typeOfTest.match(/Review flow Scenario for SSP4/i)) {
        if (itemDetails.startsWith("Sample")) {
          await correctlocator.click();
        } else if (/^Item ([1-2]|5|10|15)\b/.test(itemDetails)) {
          await incorrectlocator.click();
