...
export default class wj5NonwordRepetitionPage {
  ...
  async completeTheTakenTestForNWDREPTest(
    typeOfTest: string,
    ...
    while (await this.plainNextButtonOrEndButton.isVisible()) {
      ...
      if (typeOfTest.match(/Basal and ceiling with 6 incorrect scenario SSP1/i)) {
        ...
        } else if (/^Item [1-6]\b/.test(itemDetails)) {
            if (itemDetails.includes("Item 5")) {
                await this.audioPlayer.isEnabled();
                await this.audioPlayer.click();
            }
          await incorrectlocator.click();
        }
      } else if (typeOfTest.match(/6 correct and 6 incorrect scenario/i)) {
        if (["Introduction 1","Introduction 2","Introduction 3"].includes(itemDetails)) {
      ...
      } else if (typeOfTest.match(/6 correct and 6 incorrect scenario SSP2/i)) {
        if (["Introduction 1","Introduction 2","Introduction 3"].includes(itemDetails)) {
        }else if (itemDetails.startsWith("Sample")) {
            await correctlocator.click();
        } else if (itemDetails.startsWith("Item")) {
      ...
      } else if (typeOfTest.match(/Reverse logic with 6 incorrect scenario SSP2/i)) {
        if (["Introduction 1","Introduction 2","Introduction 3"].includes(itemDetails)) {
        }else if (["Sample Item A, Trial 1", "Sample Item A, Trial 2", "Sample Item B, Trial 1", "Sample Item B, Trial 2"].includes(itemDetails)) {
          await correctlocator.click();
        } else if (itemDetails.includes("Item 5")) {
          await incorrectlocator.click();
      ...
      } else if (typeOfTest.match(/Reverse logic with 6 incorrect scenario SSP3/i)) {
        if (["Introduction 1","Introduction 2"].includes(itemDetails)) {
        }else if (["Sample Item A, Trial 1", "Sample Item A, Trial 2", "Sample Item B, Trial 1", "Sample Item B, Trial 2"].includes(itemDetails)) {
          if (itemDetails.startsWith("Item 5")) {
            await this.audioPlayer.isEnabled();
            await this.audioPlayer.click();
          }
          await correctlocator.click();
